...
export const ImmersiveHUD = ({
  ...
  const startProgressiveSteer = useCallback((direction: 'left' | 'right') => {
    ...
    const firstAngle = Math.max(-50, Math.min(50, internalAngleRef.current + sign * STEER_INCREMENT));
    ...
    steerIntervalRef.current = setInterval(() => {
      const next = Math.max(-50, Math.min(50, internalAngleRef.current + sign * STEER_INCREMENT));
