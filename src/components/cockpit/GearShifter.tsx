...
export const GearShifter = ({ 
  ...
  const FORWARD_GEARS = new Set(["1", "2", "3", "S"]);
  ...
  return (
    <div className="flex flex-col items-center h-full pt-0.5 pb-0.5 px-1 overflow-hidden bg-gradient-to-b from-background to-background/80">
      ...
      {/* IR, SONAR, AUTOPILOT, CAMERA - All in a single horizontal line */}
      <div className="flex w-full gap-1 mb-1 justify-center items-center">
        {/* IR Button */}
        <button
          onClick={onIRToggle}
          disabled={!isEngineRunning || isAutopilotEnabled}
          className="w-11 h-11 rounded-full bg-transparent flex items-center justify-center transition-all duration-100 touch-feedback disabled:opacity-50 disabled:cursor-not-allowed"
          style={{
            outline: 'none',
            border: '1px solid rgb(20, 184, 166)'
          }}
          title={isAutopilotEnabled ? 'IR: LOCKED' : (isIREnabled ? 'IR: ON' : 'IR: OFF')}
        >
          <svg className="w-9 h-9" style={{ color: (!isEngineRunning || isAutopilotEnabled) ? 'rgb(107, 114, 128)' : (isIREnabled ? 'rgb(20, 184, 166)' : 'rgb(107, 114, 128)') }} viewBox="-5.0 -10.0 110.0 135.0">
        ...
        <button
          ...
          title={isAutopilotEnabled ? 'SONAR: LOCKED' : (isSonarEnabled ? 'SONAR: ON' : 'SONAR: OFF')}
        >
          <svg className="w-9 h-9" style={{ color: (!isEngineRunning || isAutopilotEnabled) ? 'rgb(107, 114, 128)' : (isSonarEnabled ? 'rgb(20, 184, 166)' : 'rgb(107, 114, 128)') }} viewBox="-5.0 -10.0 110.0 135.0">
        ...
        >
          <svg className="w-9 h-9" style={{ color: (!isEngineRunning || isEmergencyStop || eBrakeActive) ? 'rgb(107, 114, 128)' : (isAutopilotEnabled ? 'rgb(20, 184, 166)' : 'rgb(107, 114, 128)') }} viewBox="-5.0 -10.0 110.0 135.0">
        ...
        >
          <svg className="w-8 h-8" style={{ color: !isEngineRunning ? 'rgb(107, 114, 128)' : (isCameraEnabled ? 'rgb(20, 184, 166)' : 'rgb(107, 114, 128)') }} viewBox="0 0 24 24">
      ...
      <div className="relative w-full aspect-[110/135] mb-0.5 flex-shrink min-h-0" style={{ maxHeight: 'min(180px, 32vh)' }}>
        ...
        >
          ...
          <circle
            ...
            style={{ cursor: !isGearDisabled("S") ? 'pointer' : 'not-allowed', opacity: !isGearDisabled("S") ? 1 : 0.5, transition: 'fill 0.2s, stroke 0.2s, strokeWidth 0.2s' }}
            onClick={() => safeGearChange("S")}
          />
          <text
            x="19.4"
            y="13.1"
