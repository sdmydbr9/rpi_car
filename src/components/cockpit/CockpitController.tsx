...
export const CockpitController = () => {
  ...
  const [isWheelSyncEnabled, setIsWheelSyncEnabled] = useState(true);
  const [wheelSyncData, setWheelSyncData] = useState<TelemetryData['wheel_sync'] | null>(null);
  ...
  useEffect(() => {
    ...
    socketClient.onTelemetry((data) => {
      ...
      // Update wheel sync telemetry
      if (data.wheel_sync) {
        setWheelSyncData(data.wheel_sync);
        setIsWheelSyncEnabled(data.wheel_sync.status !== 'OFF');
      }
  ...
  const handleWheelSyncToggle = useCallback(() => {
    console.log('🔄 Wheel sync toggle');
    socketClient.emitWheelSyncToggle();
  }, []);
    ...
    <>
      ...
      <div className="h-[100dvh] w-full flex flex-col overflow-hidden">
        ...
        <Header 
          ...
          onWheelSyncToggle={handleWheelSyncToggle}
