 ...
 * @param angle - Steering angle from -50 (left) to +50 (right)
 */
export function emitSteering(angle: number): void {
  if (socket && socket.connected) {
    // Clamp angle between -50 and 50
...
/**
 * Emit gear change
 * @param gear - The gear to select: R, N, 1, 2, 3, 4
 */
export function emitGearChange(gear: string): void {
  if (socket && socket.connected) {
    const normalizedGear = gear.toUpperCase() === 'S' ? '4' : gear.toUpperCase();
    const gearLabel = { 'R': '🔙 REVERSE', 'N': '⏸️ NEUTRAL', '1': '1️⃣ 1st', '2': '2️⃣ 2nd', '3': '3️⃣ 3rd', '4': '4️⃣ 4th' }[normalizedGear] || normalizedGear;
    console.log(`[UI Control] 🎮 GEAR CHANGE: ${gearLabel}`);
    socket.emit('gear_change', { gear: normalizedGear });
  } else {
...
/**
 * Toggle closed-loop wheel speed synchronization
 */
export function emitWheelSyncToggle(): void {
  if (socket && socket.connected) {
    console.log(`[UI Control] 🔄 WHEEL SYNC: TOGGLED`);
    socket.emit('wheel_sync_toggle', {});
  } else {
    console.warn(`[UI Control] ⚠️ Cannot emit wheel sync toggle - socket not connected`);
  }
}
