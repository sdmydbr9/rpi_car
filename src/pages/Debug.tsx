...
import {
  ArrowLeft, Download, FileText, Clock, Activity, Pause, Play, Search, X, RefreshCw,
} from "lucide-react";
import {
...
// ─── time window filter ──────────────────────────────────────────
type TimeWindow = "all" | "10" | "30" | "60" | "300";
...
function useDebugData(paused: boolean, _windowSeconds: number | null) {
  ...
  useEffect(() => {
    if (status === "error" && localBufRef.current.length > 0 && localBufRef.current[0].timestamp.startsWith("20")) {
...
const Debug = () => {
  ...
  const data = useMemo(() => {
    const base = filterByWindow(samples, windowSeconds);
    if (!searchQuery) return base;
    const q = searchQuery.toLowerCase();
    return base.filter(r =>
      r.obstacle_state.toLowerCase().includes(q) || r.timestamp.includes(q),
    );
  }, [samples, windowSeconds, searchQuery]);
            ...
            <Panel title="RPM Drift (RR − RL)">
              <ResponsiveContainer width="100%" height={120}>
                <BarChart data={chartData.map(d => ({
                  t: d.t,
                  drift: +(d.rpm_rear_right - d.rpm_rear_left).toFixed(2),
                }))}>
                  <CartesianGrid strokeDasharray="2 2" stroke={C.grid} />
                  <XAxis dataKey="t" tick={tickStyle} stroke={C.axis} />
                  <YAxis tick={tickStyle} stroke={C.axis} />
                  ...
                  <Bar dataKey="drift" isAnimationActive={false}>
                    {chartData.map((d, i) => (
                      <Cell key={i} fill={(d.rpm_rear_right - d.rpm_rear_left) >= 0 ? C.teal : C.red} fillOpacity={0.7} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Panel title="Compass PID Correction (%)">
              <ResponsiveContainer width="100%" height={120}>
                <BarChart data={chartData.map(d => ({
                  t:   d.t,
                  pid: d.pid_correction,
                  err: d.heading_error_deg,
                }))}>
                  <CartesianGrid strokeDasharray="2 2" stroke={C.grid} />
                  <XAxis dataKey="t" tick={tickStyle} stroke={C.axis} />
                  <YAxis tick={tickStyle} stroke={C.axis} />
