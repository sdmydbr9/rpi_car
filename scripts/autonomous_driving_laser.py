...
from pico_sensor_reader import (
    ...
    send_pan_tilt      as pico_send_pan_tilt,
    send_center        as pico_send_center,
)
...
# Sector map
SCAN_RANGE_DEG      = 30        # ±30° from centre  → 60° total (pan/tilt gimbal range)
SCAN_STEP_DEG       = 10        # 7 sectors
...
# Laser on-demand scan (two-phase) — now via Pico pan/tilt gimbal
QUICK_SCAN_ANGLES   = [-30, -15, 0, 15, 30]   # Phase 1: quick 5-angle check
...
# Laser is on the camera pan/tilt gimbal (Pico GP2/GP3), no Pi GPIO servo
FRONT_TRIG_PIN = 25
FRONT_ECHO_PIN = 24
...
class LaserScanner:
    """On-demand directional scanner using VL53L0X on camera pan/tilt gimbal.
    ...
    within LASER_TRIGGER_DIST.  Pan servo is controlled via Pico UART.
    Two-phase scan:
      Phase 1 (quick): 5 key angles — ~250 ms
      Phase 2 (full):  7 sectors ±30° — ~350 ms extra (only if ambiguous)
    After scanning, pan returns to center.
    """
    ...
    def __init__(self):
        ...
        # Centre pan/tilt on init
        pico_send_center()

        _log.info("SCANNER_INIT on-demand mode (pan/tilt gimbal), quick_angles=%s full_sectors=%d",
                  QUICK_SCAN_ANGLES, self._n_sectors)

    # ── servo helpers (via Pico UART) ────────────────────────────────

    def _set_servo_angle(self, user_deg):
        """Move pan servo to user-space angle (-30…+30) via Pico UART.
        Pan = 90 + user_deg (center=90, tilt stays at 90)."""
        user_deg = max(-SCAN_RANGE_DEG, min(SCAN_RANGE_DEG, user_deg))
        pan = 90 + user_deg
        pico_send_pan_tilt(pan, 90)
        time.sleep(SCAN_SETTLE_MS)

    def center_servo(self):
        """Return pan/tilt to forward-facing (0°)."""
        pico_send_center()
        time.sleep(SCAN_SETTLE_MS)
...
def compute_swerve(sector_map, sweep_ready: bool = True):
    """Given the sector map, compute:
    ...
    staleness).  Forced swerve ensures the car never drives straight into
    obstacles even when threats are roughly symmetric.
    """
    ...
    # Boxed-in: trigger when surrounded.
    n_sectors = len(sector_map)
    if use_laser:
        boxed = (blocked_count >= n_sectors * 0.6)
    else:
        boxed = False
    ...
    def _drive_loop(self):
        ...
        while self._running and self.state in (State.DRIVING,
            ...
            if self.state == State.DRIVING:
                ...
                # ── Branch 1: Determine FSM zone from front sonar ──────────────
