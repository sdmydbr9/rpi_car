...
Uses ONLY sonar/laser and MPU6050 — no camera or IR.

Priority System (from slalom.py)
────────────────────────────────
  P1  ESCAPE        sonar < CRITICAL_DIST
...
class AutoPilot:
    ...
    Priority 1: ESCAPE       (dist < CRITICAL_DIST)
                              → stop → reverse → random spin → reset yaw
    ...
    # ── Distance thresholds ──
    CRITICAL_DIST    = 20     # P1: sonar < this → escape (cm)
    ...
    def _escape(self):
        ...
        stop → reverse → spin (random) → reset yaw.
        """
    ...
    def update(self):
        ...
        #    sonar < CRITICAL_DIST
        # ─────────────────────────────────────────────
        if 0 < dist < self.CRITICAL_DIST:
            print(f"\n🚨 ESCAPE ({dist:.0f}cm < {self.CRITICAL_DIST}cm)")
            self._escape()
            return
