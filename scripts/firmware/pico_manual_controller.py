"""Pico W command-only motor controller.
...
def ramp_value(applied, target):
    ...
    if abs(difference) > MAX_PWM_DELTA_PER_TICK:
        difference = MAX_PWM_DELTA_PER_TICK if difference > 0 else -MAX_PWM_DELTA_PER_TICK
...
def motor_tick(now_ms):
    ...
    if _direction_dwell:
        if time.ticks_diff(now_ms, _direction_dwell_started_ms) < DIRECTION_CHANGE_DWELL_MS:
            return
        _direction_dwell = False
...
while True:
    ...
    if (
        ...
        and time.ticks_diff(now, _last_drive_command_ms) > WATCHDOG_TIMEOUT_MS
    ):
        coast_stop()
