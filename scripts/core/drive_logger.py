...
# __file__ is scripts/core/drive_logger.py — go up 3 levels to reach rpi_car/
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
LOG_DIR = os.path.join(PROJECT_ROOT, 'rover_logs')

# CSV column order — kept flat for easy pandas consumption
# 2WD build: rear encoded motors, front Ackermann steering only
COLUMNS = [
    # Timing
    'timestamp', 'elapsed_s',
    # Source
    ...
    # Wheel encoder RPMs (rear encoded motors)
    'rpm_rear_left', 'rpm_rear_right',
    # Motor command (requested duty %)
    'cmd_pwm',
    # Motor output (applied duty % after RPM PID)
    'applied_pwm',
    # RPM PID control
