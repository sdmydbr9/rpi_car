...
class CarSystem:
    ...
    def __init__(self):
        ...
        # Closed-loop wheel speed synchronization (disabled until
        # attach_wheel_sync() is called with the RPM data source).
        self.wheel_sync = None

        # Current gear — forwarded to wheel_sync for RPM capping.
        # Updated by physics_loop / autopilot before each motor write.
        self._current_gear = "1"

    def attach_wheel_sync(self, get_rpm_fn, get_freshness_fn=None):
        """Attach closed-loop wheel speed controller.

        Call this after the Pico sensor reader is initialized so that
        the sync controller can read per-wheel RPM at 50 Hz.

        Args:
            get_rpm_fn: callable() → dict with keys
                        'rear_left', 'rear_right', 'front_right'
            get_freshness_fn: callable() → bool (True if data is recent)
        """
        from wheel_sync import WheelSpeedController
        self.wheel_sync = WheelSpeedController(
            get_rpm_fn=get_rpm_fn,
            get_duty_cap_fn=lambda: self.power_limiter.max_safe_duty,
            get_freshness_fn=get_freshness_fn,
        )
        print("✅ Wheel speed sync controller attached")

    def set_gear(self, gear: str):
        """Update current gear for RPM-limit enforcement.

        Called from physics_loop / autopilot before motor writes so the
        sync controller knows which RPM band to target.
        """
        self._current_gear = gear or "1"
        if self.wheel_sync:
            self.wheel_sync.gear = self._current_gear

    def get_sync_telemetry(self) -> dict:
        """Return per-wheel sync telemetry for the web UI."""
        if self.wheel_sync:
            return self.wheel_sync.get_telemetry()
        return {'status': 'OFF', 'wheels': {}}
    ...
    def _set_raw_motors(self, speed_l, speed_r, l_fwd, r_fwd):
        ...
        # ── Closed-loop sync path ──────────────────────────────────
        # When wheel_sync is attached and enabled, it runs per-wheel
        # PID controllers that map the commanded L/R PWM to target
        # RPMs, measure actual wheel speeds via encoders, and output
        # individually corrected per-wheel duty cycles.  The PID has
        # its own slew limiter, so we bypass the open-loop ramp below.
        if (self.wheel_sync is not None
                and self.wheel_sync.enabled
                and (speed_l > 0 or speed_r > 0)):
            fl_pwm, fr_pwm, rl_pwm, rr_pwm = self.wheel_sync.correct(
                float(speed_l), float(speed_r), duty_cap
            )
            # Clamp to duty cap and write directly (PID handles ramping)
            self._applied_duty_fl = max(0.0, min(duty_cap, fl_pwm))
            self._applied_duty_fr = max(0.0, min(duty_cap, fr_pwm))
            self._applied_duty_rl = max(0.0, min(duty_cap, rl_pwm))
            self._applied_duty_rr = max(0.0, min(duty_cap, rr_pwm))
    ...
    def brake(self):
        ...
        self._last_r_fwd = None
        # Reset wheel sync PID state after braking
        if self.wheel_sync:
            self.wheel_sync.reset()
    ...
    def stop(self):
        ...
        self._last_r_fwd = None
        # Reset wheel sync PID state so integrals don't carry over
        if self.wheel_sync:
            self.wheel_sync.reset()
