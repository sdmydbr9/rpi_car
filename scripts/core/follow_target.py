...
Public API (called by main.py):
    init(car_system, sensor_system, get_laser, get_sonar, get_gyro_z,
         get_accel_xyz, get_battery_voltage, get_current_sense)
...
# =================================================================
def init(car_system, sensor_system, get_laser, get_sonar,
         get_gyro_z_fn, get_accel_xyz_fn, get_battery_voltage_fn,
         get_current_sense_fn):
    ...
    global _get_laser, _get_sonar, _get_gyro_z, _get_accel_xyz
    global _get_battery_voltage, _get_current_sense
    global _initialized
...
# =================================================================
# MOTOR CONTROL — uses shared CarSystem (Ackermann steering)
# =================================================================

# Map follow-target steering range (±100) to servo angle.
# ±100 -> ±50° (full steering lock).
_STEER_TO_ANGLE = 50.0 / 100.0


def drive_like_a_car(throttle, steering, current_gz):
    """Ackermann drive: speed + servo steering with gyro drift correction.

    throttle: -100..+100 (positive=forward, negative=reverse)
    steering: -100..+100 (negative=left, positive=right)
    current_gz: gyro Z rate (°/s) for heading correction
    """
    ...
    # Gyro drift correction (fade out during intentional turns)
    ...
    # Convert steering to servo angle
    steer_angle = corr_steer * _STEER_TO_ANGLE

    speed = abs(throttle)
    forward = throttle >= 0

    motor_L_out, motor_R_out = throttle, corr_steer  # telemetry

    try:
        _car_system._apply_steering(speed, steer_angle, forward=forward)
    ...
    while not _shutdown_event.is_set():
        try:
            ...
            if _sensor_system and sweep_requested and not sweep_active:
                ...
                for i, angle in enumerate(range(30, -31, -10)):
                    try:
                        _sensor_system.set_servo_angle(angle)
                        time.sleep(0.04)
                        d = _sensor_system.read_laser_cm()
    ...
    while not _shutdown_event.is_set():
        try:
            ...
            # -- 3. SMOOTH SLALOM OBSTACLE AVOIDANCE --
                ...
                if avoid_state in ("SLALOM", "EMERGENCY"):
