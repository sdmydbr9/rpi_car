...
class LaserTargetingSystem:
    """Scan left/right using pan/tilt gimbal via Pico UART."""
    def __init__(self):
    ...
    def _set_angle(self, angle):
        """angle: user-space degrees, -30..+30. Maps to pan servo 60..120."""
        pan = max(60, min(120, 90 + angle))
        send_pan_tilt(pan, 90)
        time.sleep(0.15)
...
class SmartAutoPilot:
    ...
    # --------------------------------------------------
    def _decide_state(self, dist):
        # Cooldown prevents rapid state flipping
        if self.cooldown_ticks > 0:
            self.cooldown_ticks -= 1
            return self.state

        # === PANIC: extremely close ===
        if dist < 15:
            return ST_ESCAPE
    ...
    def _run_wall_follow(self, dist, gz):
    ...
    def run(self):
        ...
        while True:
            ...
            dist, gz, rpm = self._read_sensors()
            ...
            # --- Decide next state ---
            new_state = self._decide_state(dist)
