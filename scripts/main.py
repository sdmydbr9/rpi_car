...
_DEBUG_LOG_FIELDS = [
    ...
    # --- speed encoders (rear encoded motors) ---
    "rpm_rear_right", "rpm_rear_left",
    ...
    # --- motor output (2WD rear) ---
    "applied_pwm", "target_rpm", "actual_rpm",
    # --- autonomous state ---
...
_drive_logger    = None   # DriveLogger instance (only used in DEBUG_MODE)
...
def _write_debug_row():
    ...
    try:
        ...
        sync = car_system.get_sync_telemetry()
        ...
        row = [
            ...
            # motor output (2WD rear)
            round(sync.get("pwm_out", 0.0), 2),
            round(sync.get("target_rpm", 0.0), 1),
            round(sync.get("actual_rpm", 0.0), 1),
            # autonomous state
...
def _write_csv_row():
    """Log one snapshot row to CSV file during DEBUG_MODE.  Called from physics_loop at ~50 Hz."""
    if _drive_logger is None or not _drive_logger.is_running:
        return
    try:
        pkt = pico_get_sensor_packet()
        sync = car_system.get_sync_telemetry()
        compass_heading, _ = _resolve_compass_headings(
            pkt,
            car_state.get("current_heading", 0.0),
            car_state.get("target_yaw", 0.0),
        )

        # Build a dictionary matching DriveLogger.COLUMNS
        # SensorPacket fields: laser_mm, adc_a0 (battery mV), adc_a1 (current mV)
        # RPM comes from car_state (populated by encoder thread), not SensorPacket
        ...
        row = {
            ...
            'gyro_x': round(pkt.gyro_x, 3) if pkt else 0.0,
            'gyro_y': round(pkt.gyro_y, 3) if pkt else 0.0,
            'gyro_z': round(pkt.gyro_z, 3) if pkt else 0.0,
            ...
            'actual_rpm': round(sync.get("actual_rpm", 0.0), 1),
            'gear': car_state.get("gear", "?"),
            'throttle_input': round(float(car_state.get("current_pwm", 0.0) or 0.0), 2),
            'steering_input': round(float(car_state.get("steer_angle", 0) or 0), 1),
            'is_braking': int(car_state.get("is_braking", False)),
            'is_forward': int(car_state.get("gear", "?") != "R"),
            'sync_status': sync.get("status", "OFF"),
            'compass_heading': (
                compass_heading
                if compass_heading is not None
                else round(float(car_state.get("compass_heading", 0.0) or 0.0), 1)
            ),
            'pid_correction': round(float(car_state.get("pid_correction", 0.0) or 0.0), 1),
        }
        _drive_logger.log_tick(row)
    except Exception:
        pass  # Never let CSV logging crash the physics loop

# --- MOTOR PINS — L298N 2WD rear drive (Hiwonder Ackermann chassis) ---
# Managed by CarSystem in motor.py — listed here for wiring reference only.
# Rear motors + Ackermann steering servo controlled via Pico W over UART.

AVOID_SWERVE_ANGLE = 85  # Degrees to swerve when obstacle detected (increased for aggressive steering)

# --- SPEED ENCODER (Rear-Left + Rear-Right Wheel) ---
# Encoders on Pico W (GP6/GP7 left, GP8/GP9 right) — read via UART bridge
# PicoEncoderProxy provides cumulative step counts matching LgpioEncoder API
ENCODER_PPR = 330 * 4       # Pico encoder counts every edge (4× per cycle)
WHEEL_DIAMETER_M = 0.075    # Wheel diameter in meters (75mm)
import math
WHEEL_CIRCUMFERENCE_M = math.pi * WHEEL_DIAMETER_M  # ~0.2356m per revolution
...
def _wrap_heading_360(angle):
    """Normalize any angle to a compass bearing in the range [0, 360)."""
...
def _load_compass_calibration():
    ...
    try:
        ...
        calibration = {
            ...
            "scale_x": _safe_float(data.get("scale_x", 1.0), 1.0),
            "scale_y": _safe_float(data.get("scale_y", 1.0), 1.0),
        }
        print(f"🧭 [Compass] Calibration loaded: {_COMPASS_CAL_PATH}")
        return calibration
    except Exception as exc:
        print(f"⚠️  [Compass] Using raw heading (no calibration): {exc}")
        return defaults
...
def _compute_compass_heading_deg(mag_x, mag_y):
    """Convert raw magnetometer X/Y into an absolute CW compass bearing."""
    ...
    if (cx * cx + cy * cy) <= _MIN_COMPASS_VECTOR_SQ:
        return None
    # Match scripts/diagnostics/test_drive.py and the calibration utility:
    # board inversion is corrected by negating atan2, sensor mounting by +90°.
    return _wrap_heading_360(-math.degrees(math.atan2(cy, cx)) + 90.0)


def _resolve_compass_headings(packet, relative_heading=0.0, relative_target=0.0):
    """Return absolute current/target compass bearings for the UI compass."""
    if packet is None:
        return None, None

    heading = _compute_compass_heading_deg(packet.mag_x, packet.mag_y)
    if heading is None:
        return None, None

    # target_yaw/current_heading live on the autopilot's relative yaw frame.
    # Project that delta onto the absolute compass bearing so the target marker
    # still makes sense when yaw-tracking is active and overlays the current
    # heading when no course-hold target exists.
...
# current_target_rpm within this band instead of PWM%.
GEAR_RPM_RANGES = {
    "N": (0, 0),
    "1": (0, 75),
    "2": (75, 120),
...
VALID_GEARS = {"R", "N", "1", "2", "3", "4"}
FORWARD_GEARS = {"1", "2", "3", "4"}


def normalize_gear_label(gear):
    """Normalize legacy or user-supplied gear labels to the canonical set."""
    gear = str(gear or "N").upper()
    if gear == "S":
        return "4"
    return gear
...
STEER_RATE_LIMIT_PER_TICK = 30   # Max degrees steering change per 20 ms tick (1500°/s)
...
CAMERA_CONFIG_FILE = os.path.join(PROJECT_ROOT, '.camera_config.json')
...
NARRATION_CONFIG_FILE = os.path.join(PROJECT_ROOT, '.narration_config.json')
...
STARTUP_CONFIG_FILE = os.path.join(PROJECT_ROOT, '.startup_config.json')
...
class MediaMtxService:
    ...
    def __init__(self):
        ...
        self._resolved_bin = ""
        self._project_bin = os.path.join(PROJECT_ROOT, "bin", "mediamtx")
    ...
    def _build_hunter_config_text(self):
        """Config for hunter mode: plain RTSP publisher path (no rpiCamera source).

        follow_target.py pushes frames via ffmpeg to rtsp://localhost:8554/rover,
        and the HunterPopup UI reads via WebRTC WHEP at /rover/whep.
        """
        return (
            "logLevel: info\n"
            f"rtspAddress: :{self._rtsp_port}\n"
    ...
    def restart_for_hunter(self):
        """Stop MediaMTX, restart with hunter-mode config (rover path, no rpiCamera)."""
        self.stop()
        time.sleep(0.5)
        try:
            with open(self._config_path, "w") as f:
                f.write(self._build_hunter_config_text())
        except Exception as e:
            print(f"❌ [MediaMTX] Hunter config write failed: {e}")
            return False
        resolved_bin = self._resolve_bin()
        if not resolved_bin:
            print("❌ [MediaMTX] Binary not found for hunter mode")
            return False
        try:
            proc = subprocess.Popen(
            ...
            print(f"❌ [MediaMTX] Hunter start failed: {e}")
            return False
        stderr_thread = threading.Thread(target=self._drain_stderr, args=(proc,), daemon=True)
        stderr_thread.start()
        time.sleep(1.0)
        if proc.poll() is not None:
            if self._is_external_running():
                return True
            print(f"❌ [MediaMTX] Hunter process exited immediately")
            return False
        if not self._wait_ready(timeout_s=4.0):
            try:
                proc.terminate()
            except Exception:
                pass
            print("❌ [MediaMTX] Hunter mode ports not ready")
            return False
        with self._lock:
            self._proc = proc
            self._stderr_thread = stderr_thread
            self._resolved_bin = resolved_bin
        print("📡 [MediaMTX] Restarted in hunter mode (rover path, publisher source)")
        return True

    def restart_for_normal(self, width=1280, height=720, fps=30, bitrate=2500000):
        """Stop MediaMTX, restart with normal rpiCamera config."""
        self.stop()
        time.sleep(0.5)
        return self.start(width=width, height=height, fps=fps, bitrate=bitrate)
        ...
        try:
            ...
            for line in stream:
                ...
                if not txt:
                    continue
                low = txt.lower()
                # Silence frequent RTSP probe connect/disconnect info lines.
                if (
                    "[rtsp] [conn " in low
                    and " inf " in f" {low} "
                    and (" opened" in low or " closed: eof" in low)
                ):
                    continue
...
# channels, and the Pico UART bridge.  Do NOT call GPIO.setmode()
...
follow_target.init(
    ...
    get_current_sense_fn=pico_get_current_sense,
)
...
car_state = {
    ...
    "current_pwm": 0.0,   # The actual speed (0-100) right now (average of L/R)
    "current_pwm_l": 0.0, # Left wheel PWM from dual PID
    "current_pwm_r": 0.0, # Right wheel PWM from dual PID
    ...
    "compass_heading": 0.0,         # Absolute magnetometer-based compass heading (degrees CW from north)
    "compass_target_heading": 0.0,  # Absolute target heading projected into compass space
    ...
    # 🔄 Speed Encoder (Rear-Left + Rear-Right Wheel via Pico UART)
    "encoder_rpm": 0.0,                  # Average RPM from both encoders
    "encoder_rpm_left": 0.0,             # Left encoder RPM
    "encoder_rpm_right": 0.0,            # Right encoder RPM
    ...
    "dpad_rpm_preset": 0,        # D-pad RPM preset (0 = disabled, 10/25/35/50)
    # 🔄 Wheel Speed Sync (closed-loop PID)
    "wheel_sync_enabled": True,  # Master toggle for wheel speed sync
    "wheel_sync_status": "IDLE", # IDLE | ACTIVE | FALLBACK | OFF
}
...
_GAMEPAD_GEAR_MAP = {
    ...
    "3":  67.0,
    "4": 100.0,
    "S": 100.0,  # Backward-compatible alias
}
        ...
        try:
            ...
            for event in events:
                ...
                # --- Face buttons → Gear (with Select combos) ---
                    ...
                    car_state["gear"] = "1"
                    car_state["dpad_rpm_preset"] = 0
                    print("🎮 [Gamepad] Gear → 1 (max 75 RPM)")
                elif event.code in ('BTN_EAST', 'BTN_B') and event.state == 1:
                    car_state["gamepad_gear"] = "2"
                    car_state["gear"] = "2"
                    car_state["dpad_rpm_preset"] = 0
                    print("🎮 [Gamepad] Gear → 2 (max 120 RPM)")
                    ...
                    if _gamepad_btn_tl2_held and _gamepad_btn_tr2_held:
                        ...
                        car_state["gear"] = "3"
                        car_state["dpad_rpm_preset"] = 0
                        print("🎮 [Gamepad] Gear → 3 (max 200 RPM)")
                elif event.code in ('BTN_NORTH', 'BTN_Y') and event.state == 1:
                    car_state["gamepad_gear"] = "4"
                    car_state["gear"] = "4"
                    car_state["dpad_rpm_preset"] = 0
                    print("🎮 [Gamepad] Gear → 4 (max 300 RPM)")
                ...
                # --- D-pad → RPM presets ---
                    ...
                    car_state["dpad_rpm_preset"] = 10
                    print("🎮 [Gamepad] D-pad ↓  RPM preset → 10")
                elif event.code == 'ABS_HAT0X' and event.state == 1:
                    car_state["dpad_rpm_preset"] = 25
                    print("🎮 [Gamepad] D-pad →  RPM preset → 25")
                elif event.code == 'ABS_HAT0Y' and event.state == -1:
                    car_state["dpad_rpm_preset"] = 35
                    print("🎮 [Gamepad] D-pad ↑  RPM preset → 35")
                elif event.code == 'ABS_HAT0X' and event.state == -1:
                    car_state["dpad_rpm_preset"] = 50
                    print("🎮 [Gamepad] D-pad ←  RPM preset → 50")

                # --- L3 (BTN_THUMBL) / R3 (BTN_THUMBR) → Brake ---
                elif event.code in ('BTN_THUMBL', 'BTN_THUMBR'):
                    _gamepad_handle_thumb_brake(event.code, event.state)
...
# ------------------------------------------------------------------
# Helper: L3 / R3 thumb-stick press → normal brake (hold to brake)
# ------------------------------------------------------------------
def _gamepad_handle_thumb_brake(code, state):
    """Apply normal braking when either thumb-stick button is pressed.

    Press (state=1) → brake on, Release (state=0) → brake off.
    Mirrors the UI brake pedal behaviour (sets brake_pressed / is_braking).
    """
    label = 'L3' if code == 'BTN_THUMBL' else 'R3'
    if state == 1:
        car_state["gas_pressed"] = False
        car_state["brake_pressed"] = True
        car_state["is_braking"] = True
        ...
        try:
            ...
            pass
        print(f"🎮 [Gamepad] {label} → BRAKE: 🔴 ON")
    else:
        car_state["brake_pressed"] = False
        car_state["is_braking"] = False
        print(f"🎮 [Gamepad] {label} → BRAKE: 🟢 OFF")
...
def _gamepad_drive_thread():
    """
    Runs at ~50 Hz.  When engine is running and a gamepad is connected,
    ...
    while True:
        if car_state["engine_running"] and car_state["gamepad_connected"]:
            raw_throttle = car_state["gamepad_throttle"]
            raw_steering = car_state["gamepad_steering"]

            # Exponential smoothing (moderate for throttle, raw pass-through for steering)
            smoothed_throttle = 0.4 * raw_throttle + 0.6 * smoothed_throttle
            # Steering: no smoothing — pass raw value for instant response
            ...
            # Map steering (-100..100) → angle (-50..50)
            # Direct mapping — physics_loop already rate-limits for motor safety
            angle = int(max(-50, min(50, smoothed_steering * 0.5)))
            car_state["user_steer_angle"] = angle
            # Also update steer_angle directly so physics_loop sees it
            # immediately (obstacle avoidance may overwrite later)
            if car_state.get("obstacle_state", "IDLE") == "IDLE":
                car_state["steer_angle"] = angle

        elif car_state["gamepad_connected"] and not car_state["engine_running"]:
            # Gamepad connected but engine off → idle
            smoothed_throttle = 0.0
            smoothed_steering = 0.0
...
def physics_loop():
    ...
    # Friction coast timeout — max 2 seconds of unpowered rolling
    _coast_start_time = None     # monotonic time when coast began (gas released)
    FRICTION_COAST_MAX_S = 2.0   # Hard limit: force brake after this many seconds

    # Direction change detection — reset RPM target when forward↔reverse
    _prev_is_forward = True

    # Dual PID per-wheel PWM output (from car_system.rpm_pid_tick)
    _pid_pwm_l = 0.0
    _pid_pwm_r = 0.0
    ...
    while True:
        ...
        if VISION_AVAILABLE and vision_system is not None and not car_state["autonomous_mode"]:
            ...
            is_forward_gear = normalize_gear_label(gear_val) in FORWARD_GEARS
        ...
        # When gear switches between forward and reverse, immediately
        # zero the RPM target and PWM to prevent the PID from driving
        # the car in the new direction with the old target.
        _cur_is_forward = (gear != "R")
        if _cur_is_forward != _prev_is_forward:
            car_state["current_target_rpm"] = 0.0
            car_state["current_pwm"] = 0
            _pid_pwm_l = 0.0
            _pid_pwm_r = 0.0
            _coast_start_time = None
            car_system.rpm_pid_tick(0)  # flush PID integral state
        _prev_is_forward = _cur_is_forward

        # --- STEERING: INSTANT PASS-THROUGH ---
        # No rate-limiting — steering must be snappy and immediate.
        # The binary 0/max differential mixer doesn't need gradual
        # angle transitions; it only checks > or < ±2°.
        user_angle = int(user_angle)
        # Use the user angle directly for the rest of this tick
        # (obstacle avoidance may override steer_angle below)
        ...
        if emergency_laser_override:
            ...
            car_state["current_pwm"] = 0
            car_state["current_target_rpm"] = 0
            car_state["is_braking"] = True
            ...
            # In autonomous/hunter mode, the module owns its own FSM —
        ...
        if obstacle_state == "IDLE":
            ...
            if car_state["autonomous_mode"] or car_state["hunter_mode"]:
                ...
                if car_state["emergency_brake_active"]:
                    ...
                    # AutoPilot/Hunter controls motors directly via CarSystem.
                ...
                # BRAKE PEDAL PRESSED - Real car braking behavior
                    ...
                    _coast_start_time = None  # Reset coast timer on active brake
                    # Ramp RPM target down smoothly instead of instant zero.
                    # Magnetic brake engages only once RPM target reaches near-zero.
                    target_rpm = car_state.get("current_target_rpm", 0.0)
                    if target_rpm > 5.0:
                        target_rpm -= BRAKE_DECEL_RATE * 15  # RPM decel (~45 RPM/tick)
                        target_rpm = max(0, target_rpm)
                        car_state["current_target_rpm"] = target_rpm
                        # Dual PID drives motors to match target RPM (independent L/R)
                        _pid_pwm_l, _pid_pwm_r = car_system.rpm_pid_tick(target_rpm)
                        car_state["current_pwm"] = (_pid_pwm_l + _pid_pwm_r) / 2.0
                        car_state["current_pwm_l"] = round(_pid_pwm_l, 1)
                        car_state["current_pwm_r"] = round(_pid_pwm_r, 1)
                    ...
                    car_state["is_braking"] = False
                    # RPM-based gear control: each gear defines (min_rpm, max_rpm)
                    min_rpm, max_rpm = GEAR_RPM_RANGES.get(gear, (0, 100))
                    target_rpm = car_state.get("current_target_rpm", 0.0)

                    # D-pad RPM preset overrides gear max when active
                    dpad_preset = car_state.get("dpad_rpm_preset", 0)
                    if dpad_preset > 0:
                        max_rpm = dpad_preset

                    if gas and gear != "N":
                        _coast_start_time = None  # Reset coast timer when throttle active
                        # Throttle pressed: ramp toward max_rpm for current gear
                        # (or D-pad preset if active).
                        # Smooth ramp from 0 (no instant jump to min_rpm) so the
                        ...
                        # coasting.  After that, force-brake to stop the car.
                        # Uses encoder + IMU to detect if the car is still moving.
                        if target_rpm <= 0 and car_system.is_car_moving():
                            if _coast_start_time is None:
                                _coast_start_time = time.monotonic()
                            elif (time.monotonic() - _coast_start_time) >= FRICTION_COAST_MAX_S:
                                car_state["is_braking"] = True
                                _coast_start_time = None
                        elif target_rpm <= 0:
                            _coast_start_time = None
        ...
        if car_state["autonomous_mode"] or car_state["hunter_mode"] or rth_running:
            ...
            auto_gear = "4" if car_state["hunter_mode"] else "3"
            car_system.set_gear(auto_gear)
            time.sleep(0.02)
            continue
        ...
        # Re-read current_pwm since RPM PID may have updated it.
        current = car_state["current_pwm"]
        try:
...
# loop into an in-memory ring buffer AND to CSV files.
if DEBUG_MODE:
    try:
        import sys as _sys
        ...
        # Also initialize CSV logging for debug sessions
        _drive_logger = DriveLogger(source="debug")
        _drive_logger.start()
...
@app.route("/gear/<gear>")
def set_gear(gear):
    """Sets the gear (R, N, 1, 2, 3, 4)."""
    gear = normalize_gear_label(gear)
...
def on_steering(data):
    """Handle steering control (angle -50 to +50)"""
    global _last_steering_log_ts, _last_steering_log_bucket
    try:
        angle = int(data.get('angle', 0))
        angle = max(-50, min(50, angle))  # Clamp to -50 to 50
        car_state["user_steer_angle"] = angle
        if abs(angle) > 10:
            car_state["turning"] = "left" if angle < 0 else "right"
...
def on_gear_change(data):
    ...
    """
    gear = normalize_gear_label(data.get('gear', 'N'))
    if gear in VALID_GEARS:
        ...
        if switching_direction and current_speed > MIN_SPEED_FOR_GEAR_CHANGE:
            ...
            # Even at low speed, zero the RPM target to prevent residual
            # PID output from driving the car in the new direction.
            car_state["current_target_rpm"] = 0.0
            car_state["current_pwm"] = 0
...
@socketio.on('wheel_sync_toggle')
def on_wheel_sync_toggle(data):
    """Toggle closed-loop wheel speed synchronization on/off."""
    if car_system.wheel_sync is None:
        emit('wheel_sync_response', {
            'status': 'error',
            'wheel_sync_enabled': False,
            'message': 'Wheel sync not available (no Pico sensor bridge)'
        })
        return
    new_state = not car_system.wheel_sync.enabled
    car_system.wheel_sync.enabled = new_state
    car_state["wheel_sync_enabled"] = new_state
    car_state["wheel_sync_status"] = car_system.wheel_sync.status
    state_str = '✅ ON' if new_state else '❌ OFF'
    print(f"\n⚙️ [UI Control] 🔄 WHEEL SYNC: {state_str}")
    emit('wheel_sync_response', {
        'status': 'ok',
...
# ==========================================
# 🔄 SPEED ENCODER + POWER TELEMETRY
# ==========================================
# Encoders now on Pico (GP3/GP2 left, GP4/GP5 right) — RPM calculated on Pico
# PicoEncoderProxy still available for step counts; RPM read directly from packet
try:
    from pico_sensor_reader import get_encoder_proxy_left, get_encoder_proxy_right, get_pico_rpm
    _encoder_left  = get_encoder_proxy_left()
    _encoder_right = get_encoder_proxy_right()
    if _encoder_left and _encoder_right:
        car_state["encoder_available"] = True
        print("🔄 [Encoder] ✅ Encoders initialised (Pico UART) — Left (GP3/GP2), Right (GP4/GP5)")
        # Attach proxy encoder objects to car_system for direct step-based RPM reading
        # (rpm_pid_tick reads .steps each tick — proxied from Pico cumulative counts)
        car_system.attach_wheel_sync(_encoder_left, _encoder_right)
        # Attach IMU accelerometer for motion detection
        car_system.attach_imu(pico_get_accel_xyz)
    else:
        car_state["encoder_available"] = False
        print("⚠️  [Encoder] Pico encoder proxies not available (reader not initialised)")
...
def telemetry_broadcast():
    ...
    while True:
        try:
            ...
            target_rpm = car_state.get("current_target_rpm", 0.0)
            rpm_left = car_state.get("encoder_rpm_left", 0.0)
            rpm_right = car_state.get("encoder_rpm_right", 0.0)
            
            # 📡 Get fresh IMU + compass data from Pico
            pico_pkt = pico_get_sensor_packet()
            if pico_pkt:
                accel_x, accel_y, accel_z = pico_pkt.accel_x, pico_pkt.accel_y, pico_pkt.accel_z
            else:
                accel_x, accel_y, accel_z = 0.0, 0.0, 0.0

            compass_heading, compass_target_heading = _resolve_compass_headings(
                pico_pkt,
                car_state.get("current_heading", 0.0),
                car_state.get("target_yaw", 0.0),
            )
            if compass_heading is not None:
                car_state["compass_heading"] = compass_heading
            if compass_target_heading is not None:
                car_state["compass_target_heading"] = compass_target_heading
            ...
            telemetry_data = {
                ...
                "current_pwm": pwm,
                "current_pwm_l": car_state.get("current_pwm_l", 0.0),
                "current_pwm_r": car_state.get("current_pwm_r", 0.0),
                "gear": car_state["gear"],
                ...
                "pid_correction": car_state.get("pid_correction", 0.0),
                "gyro_available": car_state["gyro_available"],
                "gyro_calibrated": car_state["gyro_calibrated"],
                # 🧭 Absolute compass telemetry (available even without course correction)
                "compass_heading": car_state.get("compass_heading", 0.0),
                "compass_target_heading": car_state.get(
                    "compass_target_heading",
                    car_state.get("compass_heading", 0.0),
                ),
                # 🧭 Slalom yaw-tracking telemetry
...
def telemetry():
    ...
    return jsonify({
        ...
        "rpm_left": car_state.get("encoder_rpm_left", 0.0),
        "rpm_right": car_state.get("encoder_rpm_right", 0.0),
        "target_rpm": round(car_state.get("current_target_rpm", 0.0), 1),
        "speed_mpm": real_speed_mpm,
        "encoder_available": car_state["encoder_available"],
        "current_pwm": pwm,
        "gear": car_state["gear"],
        ...
        "compass_heading": (
            compass_heading
            if compass_heading is not None
            else car_state.get("compass_heading", 0.0)
        ),
        "compass_target_heading": (
            compass_target_heading
            if compass_target_heading is not None
            else car_state.get("compass_target_heading", car_state.get("compass_heading", 0.0))
        ),
        # 🔋 Battery / power telemetry
        "battery_voltage": car_state.get("battery_voltage", -1),
        "current_amps": car_state.get("current_amps", -1),
...
# ==========================================
# 🎯 HUNTER MODE (follow_target module)
# ==========================================
_hunter_lock = threading.Lock()

def _start_hunter():
    """Activate the follow_target module, releasing the main camera first.
    
    Uses the shared CarSystem (no separate GPIO). Mutually exclusive with autopilot.
    Camera handoff sequence:
      1. Stop autopilot if active
      2. Release integrated Picamera2
      3. Stop H264 RTSP publisher
      4. Restart MediaMTX in hunter mode (rover path, RTSP publisher source)
      5. Start follow_target (grabs Picamera2 + ffmpeg → rtsp://localhost:8554/rover)
      6. UI connects via WHEP at /rover/whep
    """
    with _hunter_lock:
        if follow_target.is_active():
            return True
        # Disable autopilot if active (mutually exclusive)
        if car_state.get("autonomous_mode", False):
            car_state["autonomous_mode"] = False
            autopilot.stop()
            car_system.stop()
            print("🎯 [Hunter] Disabled autopilot (mutually exclusive)")
        print("🎯 [Hunter] Releasing camera for hunter mode...")
        _pause_integrated_camera_for_h264()
        _h264_rtsp_service.stop()
        time.sleep(0.3)
        # Restart MediaMTX with hunter config (rover path, no rpiCamera source)
        print("🎯 [Hunter] Restarting MediaMTX for hunter stream...")
        if not _mediamtx_service.restart_for_hunter():
            print("⚠️  [Hunter] MediaMTX hunter restart failed, continuing anyway")
        try:
            car_state["hunter_mode"] = True
            follow_target.start()
            print("🎯 [Hunter] follow_target module started (shared CarSystem)")
            return True
        except Exception as e:
            print(f"❌ [Hunter] Failed to start: {e}")
            car_state["hunter_mode"] = False
            # Restore normal MediaMTX + camera
            _mediamtx_service.restart_for_normal()
            _resume_integrated_camera_from_h264(force=True)
            return False

def _stop_hunter():
    """Deactivate the follow_target module and resume main camera.
    
    Restore sequence:
      1. Stop follow_target (releases Picamera2 + kills ffmpeg)
      2. Restart MediaMTX with normal rpiCamera config
      3. Resume integrated camera
    """
    with _hunter_lock:
        car_state["hunter_mode"] = False
        if follow_target.is_active():
            print("🎯 [Hunter] Stopping follow_target module...")
            follow_target.stop()
            time.sleep(0.5)
        # Reset car state to safe manual mode
        car_state["current_pwm"] = 0
        car_state["gas_pressed"] = False
        car_state["brake_pressed"] = False
        car_state["gear"] = "N"
        car_state["steer_angle"] = 0
        car_state["user_steer_angle"] = 0
        # Restore normal MediaMTX (rpiCamera source) + integrated camera
        print("🎯 [Hunter] Restoring MediaMTX + camera...")
        _mediamtx_service.restart_for_normal()
        _resume_integrated_camera_from_h264(force=True)


# --- Hunter Flask routes (direct module calls) ---

@app.route("/api/hunter/status")
def hunter_status_route():
    if not follow_target.is_active():
        return jsonify({"error": "hunter not active"}), 503
    return jsonify(follow_target.get_status())

@app.route("/api/hunter/touch", methods=["POST"])
def hunter_touch_route():
    if not follow_target.is_active():
        return jsonify({"error": "hunter not active"}), 503
    data = request.get_json(force=True)
    result = follow_target.handle_touch(
        float(data.get('x', 0.5)),
        float(data.get('y', 0.5)),
    )
    return jsonify(result)

@app.route("/api/hunter/command", methods=["POST"])
def hunter_command_route():
    if not follow_target.is_active():
        return jsonify({"error": "hunter not active"}), 503
    data = request.get_json(force=True)
    result = follow_target.handle_command(data.get('cmd', ''))
    return jsonify(result)

@app.route("/api/hunter/template.jpg")
def hunter_template_route():
    if not follow_target.is_active():
        return Response(b'', status=503)
    jpg = follow_target.get_template_jpg()
    if jpg is None:
        return Response(b'', status=204)
    return Response(jpg, status=200, mimetype='image/jpeg')


# --- Hunter socket events ---

@socketio.on('hunter_activate')
def handle_hunter_activate(data=None):
    """Start follow_target module for target pursuit mode."""
    print("🎯 [Hunter] Activation requested via socket")
    def _activate():
        ok = _start_hunter()
        socketio.emit('hunter_status', {'active': ok})
    threading.Thread(target=_activate, daemon=True, name="Thread-HunterActivate").start()

@socketio.on('hunter_deactivate')
def handle_hunter_deactivate(data=None):
    """Stop follow_target module and resume normal operation."""
    print("🎯 [Hunter] Deactivation requested via socket")
    def _deactivate():
        _stop_hunter()
        socketio.emit('hunter_status', {'active': False})
    threading.Thread(target=_deactivate, daemon=True, name="Thread-HunterDeactivate").start()
...
if __name__ == "__main__":
    ...
    try:
        ...
        autopilot.stop()
        car_system.cleanup()
        # Pico encoder proxies need no cleanup (UART handles it)
        if _encoder_left:
            _encoder_left.cancel()
        if _encoder_right:
            _encoder_right.cancel()
