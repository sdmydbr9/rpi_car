...
# --- GYRO STRAIGHT-LINE HEADING CORRECTION (MPU6500 via Pico) ---
...
# Attach closed-loop wheel speed synchronization controller.
# Uses per-wheel PID + feedforward to compensate for plastic (rear)
# vs metal (front) gearbox speed differences.  Runs inside
# CarSystem._set_raw_motors() at 50 Hz — transparent to all callers.
try:
    if pico_reader is not None:
        car_system.attach_wheel_sync(
            get_rpm_fn=pico_get_rpm,
            get_freshness_fn=lambda: (
                pico_reader is not None
                and pico_reader.is_connected()
                and pico_reader.is_fresh(max_age_s=1.0)
            ),
        )
    else:
        print("⚠️  Wheel sync disabled — no Pico sensor bridge")
except Exception as e:
...
def physics_loop():
    ...
    while True:
        ...
        if car_state["autonomous_mode"] or car_state["hunter_mode"]:
            ...
            # Autopilot uses gear "3" (fast driving), hunter uses "S" (sport).
            auto_gear = "S" if car_state["hunter_mode"] else "3"
        ...
        # HARD DIFFERENTIAL: any steering input beyond 2° deadband
        # immediately sets the inner side to 0 for effective turning.
        if angle < -2: # TURNING LEFT — stop left wheels
            left_motor_speed = 0
            right_motor_speed = current
            
        elif angle > 2: # TURNING RIGHT — stop right wheels
            right_motor_speed = 0
            left_motor_speed = current
        ...
        try:
            # Feed current gear to wheel sync for RPM-limit enforcement
