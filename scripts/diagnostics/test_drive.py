...
# Loaded from compass_cal.json (written by compass_calibrate_and_test.py).
# Defaults are identity (no correction) until the file is read in main().
_MAG_OFFSET_X = 0.0
_MAG_OFFSET_Y = 0.0
_MAG_SCALE_X  = 1.0
_MAG_SCALE_Y  = 1.0
...
def _load_calibration() -> bool:
    """Read compass_cal.json into the module-level correction globals.
...
#  MATHS HELPERS
# ══════════════════════════════════════════════════════════════════════════════
def compute_heading(mag_x: float, mag_y: float) -> float:
    """Return a calibrated CW compass bearing (0-360°).

    Applies the hard-iron offsets and soft-iron scale corrections loaded from
    compass_cal.json.  The formula matches compass_calibrate_and_test.py:
      - subtract hard-iron centre offset
      - apply soft-iron axis scale
      - negate atan2 to convert math-CCW to compass-CW
      - +90° corrects for the sensor being mounted 90° CCW on the board
    """
    cx = (mag_x - _MAG_OFFSET_X) * _MAG_SCALE_X
    cy = (mag_y - _MAG_OFFSET_Y) * _MAG_SCALE_Y
...
def calibrate_heading(pico: "PicoSensorReader", n: int = CALIB_FRAMES) -> float:
    ...
    while len(samples) < n and time.monotonic() < deadline:
        pkt = pico.get_latest()
        if pkt:
            samples.append(compute_heading(pkt.mag_x, pkt.mag_y))
...
def drive_loop(stdscr, pico, car, initial_target_heading, logger=None):
    ...
    while True:
        ...
        pkt = pico.get_latest()
        if pkt:
            current_hdg = compute_heading(pkt.mag_x, pkt.mag_y)
            gyro_z      = pkt.gyro_z
            mx, my, mz  = pkt.mag_x, pkt.mag_y, pkt.mag_z
            ax, ay, az  = pkt.accel_x, pkt.accel_y, pkt.accel_z
            rpm_rr      = pkt.rpm_rear_right
            rpm_rl      = pkt.rpm_rear_left
            rpm_fr      = pkt.rpm_front_right
...
def main():
    ...
    if _load_calibration():
        ...
        print(f"   offset=({_MAG_OFFSET_X:.2f}, {_MAG_OFFSET_Y:.2f})  "
