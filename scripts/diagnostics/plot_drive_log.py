...
num_cols = [
    ...
    'rpm_rear_left', 'rpm_rear_right',
    'cmd_pwm', 'applied_pwm',
    'target_rpm', 'actual_rpm',
    'throttle_input', 'steering_input',
    'compass_heading', 'pid_correction',
]
for c in num_cols:
    if c in raw.columns:
...
if 'compass_heading' in raw.columns:
    print("  (Compass course correction removed — new Hiwonder chassis drives straight)")
...
# ── FIGURE LAYOUT ─────────────────────────────────────────────────
fig = plt.figure(figsize=(18, 30), facecolor=DARK)
fig.suptitle(
...
if 'target_rpm' in raw.columns:
    ax2.plot(t, raw['target_rpm'], color='white', linewidth=1.5,
             linestyle='--', alpha=0.6, label='Target')
ax2.plot(t, raw['rpm_rear_left'], color=BLUE, linewidth=1.2, label='RL')
ax2.plot(t, raw['rpm_rear_right'], color=ORANGE, linewidth=1.2, label='RR')
...
if 'target_rpm' in raw.columns:
    err_rl = raw['target_rpm'] - raw['rpm_rear_left']
    ax3.plot(t, err_rl, color=BLUE, linewidth=1, alpha=0.85, label='RL Error')
    err_rr = raw['target_rpm'] - raw['rpm_rear_right']
...
style_ax(ax4, '4. Applied PWM (PID Output)', 'PWM %')

if 'applied_pwm' in raw.columns:
    ax4.plot(t, raw['applied_pwm'], color=BLUE, linewidth=1.2, label='Applied PWM')
if 'cmd_pwm' in raw.columns:
    ax4.plot(t, raw['cmd_pwm'], color='white', linewidth=1, linestyle='--',
             alpha=0.5, label='Commanded')
...
if 'cmd_pwm' in raw.columns:
    ax5.plot(t, raw['cmd_pwm'], color='white', linewidth=1.5, linestyle='--',
             alpha=0.6, label='Commanded')
if 'applied_pwm' in raw.columns:
    ax5.plot(t, raw['applied_pwm'], color=BLUE, linewidth=1, alpha=0.9,
             label='Applied')
